...
function set_root_dir {
  if [[ -z "${ROOT_MODULE}" ]]; then
    echo "ERROR: ROOT_MODULE is not set!"
    return 1
  fi
...
function log_error {
  ...
  >&2 echo -e "${COLOR_BOLD}${COLOR_RED}[ERROR] ${current_time} $@${COLOR_NONE}"
}

function log_warning {
  local current_time
  current_time=$(date "+%Y-%m-%d %H:%M:%S")
  >&2 echo -e "${COLOR_ORANGE}[WARN] ${current_time} $@${COLOR_NONE}"
}

function log_callout {
  local current_time
  current_time=$(date "+%Y-%m-%d %H:%M:%S")
  >&2 echo -e "${COLOR_LIGHTCYAN}[CALLOUT] ${current_time} $@${COLOR_NONE}"
}

function log_cmd {
  local current_time
  current_time=$(date "+%Y-%m-%d %H:%M:%S")
  >&2 echo -e "${COLOR_BLUE}[CMD] ${current_time} $@${COLOR_NONE}"
}

function log_success {
  local current_time
  current_time=$(date "+%Y-%m-%d %H:%M:%S")
  >&2 echo -e "${COLOR_GREEN}[SUCCESS] ${current_time} $@${COLOR_NONE}"
}
...
function run {
  ...
  if [ ${error_code} -ne 0 ]; then
    ...
    return ${error_code}
    # false # Failed without exiting the shell
  fi
}
